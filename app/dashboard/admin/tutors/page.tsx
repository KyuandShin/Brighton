...
export default function AdminTutorsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case 'PENDING':
        return <span className="px-3 py-1 bg-yellow-500/10 text-yellow-500 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center gap-1">
          <Clock size={12} /> Pending
        </span>;
      case 'APPROVED':
        return <span className="px-3 py-1 bg-green-500/10 text-green-500 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center gap-1">
          <Check size={12} /> Approved
        </span>;
      case 'REJECTED':
        return <span className="px-3 py-1 bg-red-500/10 text-red-500 rounded-full text-[10px] font-black uppercase tracking-wider flex items-center gap-1">
          <X size={12} /> Rejected
        </span>;
      default:
        return null;
    ...
    <div className="space-y-8">
      <header className="space-y-2">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-[#748ffc]/10 rounded-2xl flex items-center justify-center">
            <Shield size={24} className="text-[#748ffc]" />
          </div>
          <div>
            <h2 className="text-3xl font-black tracking-tight text-[#2c3e50]">Tutor Management</h2>
            <p className="text-[#7f8c8d] font-bold text-xs uppercase tracking-widest">
              Review and approve tutor applications
            </p>
          </div>
          {pendingCount > 0 && (
            <span className="px-4 py-1.5 bg-red-500 text-white rounded-full text-[10px] font-black uppercase animate-pulse">
          ...
          {(['ALL', 'PENDING', 'APPROVED', 'REJECTED'] as const).map((status) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                filter === status
                  ? 'bg-white text-[#748ffc] shadow-md'
                  : 'text-[#adb5bd] hover:text-[#2c3e50]'
              }`}
            >
              {status}
            </button>
      ...
      {loading ? (
        ...
        <div className="bg-white border-2 border-dashed border-[#e9ecef] rounded-[40px] p-16 text-center space-y-4">
          <UserCheck size={40} className="mx-auto text-[#adb5bd]" />
          ...
          {filtered.map((tutor, i) => (
            <div 
              ...
              className={`p-6 border-b border-[#f1f3f5] last:border-0 ${
                tutor.verificationStatus === 'PENDING' ? 'bg-yellow-500/5' : ''
              }`}
            >
              <div className="flex flex-col md:flex-row md:items-center gap-6">
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div className="space-y-1 min-w-0">
                    ...
                    <p className="text-xs text-[#7f8c8d] line-clamp-1">
                      {tutor.headline}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  <Link 
                    ...
                    className="p-2.5 bg-[#f8f9fa] rounded-xl hover:bg-[#748ffc] hover:text-white transition-all"
                  >
                    <Eye size={16} />
                  </Link>
                  ...
                  {tutor.verificationStatus === 'PENDING' && (
                    <>
                      <button
                        ...
                        className="p-2.5 bg-green-500/10 text-green-500 rounded-xl hover:bg-green-500 hover:text-white transition-all disabled:opacity-50"
                      >
                        <UserCheck size={16} />
                      </button>
                      <button
