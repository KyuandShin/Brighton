...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Incorrect answers for AI context ───────────────────────────────────
    const incorrectItems = graded.filter(g => !g.correct).map(g => {
      const q = lookup.get(g.questionId);
      return `- "${q?.text}" → Student answered: "${g.studentAnswer}" | Correct: "${g.correctAnswer}" | Topic: ${g.topic}`;
    }).join('\n');

    const topicSummary = topicResults.map(t =>
      `${t.topic}: ${t.correct}/${t.total} (${t.accuracy}%)`
    ).join('\n');

    // ── Call Claude AI for analysis ─────────────────────────────────────────
    const aiPrompt = `You are an expert Philippine K-12 academic advisor for Brighton Tutoring Platform.

A ${level === 'HIGH_SCHOOL' ? 'high school' : 'elementary'} student just completed a placement assessment.
...
${incorrectItems || 'None — perfect score!'}

Analyze this student's performance and respond ONLY with valid JSON (no markdown, no extra text):
{
  "mastery_level": "one of: Beginner | Developing | Proficient | Advanced",
  "weaknesses": [
    {
      "topic": "exact topic name from breakdown",
      "accuracy": number (0-100),
      "proficiency": "short label like Needs Work | Struggling | Below Average",
      "priority": number (1=most urgent, 2=important, 3=moderate),
      "tip": "one specific, actionable study tip for this topic in 1 sentence"
    }
  ],
  "strengths": [
    {
  ...
  "recommendation": "Write 2-3 warm, encouraging, personalized sentences. Mention specific weak subjects by name. Suggest what kind of tutor they should look for. Use a supportive tone like a caring teacher would. Include 1-2 relevant emojis naturally.",
  "study_plan": "Write a brief 2-sentence personalized study plan focusing on the top 2 weak areas."
}

Rules:
- weaknesses = topics with accuracy below 70%, sorted by priority (lowest accuracy first)
- strengths = topics with accuracy 80% or above
- If score is perfect, weaknesses = [] and write a celebratory recommendation
- Return ONLY the JSON object, nothing else`;
    ...
    const rawText = aiData.content?.find((b: { type: string }) => b.type === 'text')?.text ?? '{}';

    let analysis: {
      mastery_level: string;
      weaknesses: { topic: string; accuracy: number; proficiency: string; priority: number; tip: string }[];
    ...
    try {
      ...
      // Fallback if AI JSON is malformed
      analysis = {
        mastery_level: percentage >= 80 ? 'Proficient' : percentage >= 60 ? 'Developing' : 'Beginner',
        weaknesses: topicResults.filter(t => t.accuracy < 70).sort((a, b) => a.accuracy - b.accuracy).map((t, i) => ({
          topic: t.topic, accuracy: t.accuracy, proficiency: t.accuracy < 40 ? 'Struggling' : 'Needs Work', priority: i + 1, tip: `Review your notes and practice more problems on ${t.topic}.`,
        })),
        strengths: topicResults.filter(t => t.accuracy >= 80).map(t => ({ topic: t.topic, accuracy: t.accuracy })),
        recommendation: `You scored ${percentage}% on your placement test. ${percentage >= 70 ? "Great effort! Keep it up! 🌟" : "Don't worry — that's what tutors are for! 💪"} Find a tutor who specializes in your weak subjects to help you improve.`,
        study_plan: `Focus on your weakest topics first by reviewing notes and doing practice problems daily. Consider booking a tutor session at least twice a week.`,
      };
    }
